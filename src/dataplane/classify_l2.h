...
//   C2 (this)   — compound hash lookup via rte_hash_lookup_data,
...
//   3. If outer ethertype == 0x8100: extract vlan_id and pcp from
//      the 802.1Q TCI; inner ethertype is at offset 16. (Minimal
//      VLAN id extraction for selectivity probing — l3_offset dynfield
//      write is deferred to C3.)
//   4. Probe in selectivity order (§5.2): src_mac → dst_mac → vlan →
//      ethertype → pcp. Each probe: rte_hash_lookup_data, filter_mask
//      check, dispatch on first match.
//   5. Miss: return kNextL3.
//
// TODO C3: l3_offset dynfield write per D13.
...
inline ClassifyL2Verdict classify_l2(struct rte_mbuf* m,
  ...
  // VLAN extraction (minimal — for selectivity probe key only; l3_offset
  // write is C3's responsibility).
