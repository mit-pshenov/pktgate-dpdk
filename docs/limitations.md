...
**Отложено** (internal tracking id: M12). Design есть (секция §12 в
design.md), реализация — нет.
...
shipped build'е **runtime enforcement не wired** (Phase 1 scope-trim,
post-phase1 debt).
...
**Отложено.** В текущем build'е shipped **deep-copy mirror** (`rte_pktmbuf_copy`).
...
под test harness, см. `src/main.cpp:419`), но в коде нет проверки на
...
3. **Для prod-grade deployment'ов** — provide feedback через PR /
   issue'сы с конкретным use case'ом. Архитектурный growth есть
   (rte_flow hw offload, mirror zero-copy, HA / watchdog) — в
   roadmap'е, вопрос приоритизации.
