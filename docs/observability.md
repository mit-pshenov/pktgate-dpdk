...
— watchdog/bypass относятся к deferred HA-функциональности, см.
`docs/limitations.md`). Таблицы ниже помечают такие row'ы как
**exposed: no** — оператор не увидит их в scrape'е.
...
| `pktgate_lcore_cycles_per_burst` | H | `lcore` | **exposed: no** — в manifest как histogram, но emit path в Phase 1 не wired. SLO-критический (N2 ≤500 µs p99) — пока backport не сделан, latency бюджет валидируется только по pre-ship bench-артефактам, не live-scrape. |
...
| `pktgate_mirror_dropped_total` | C | `target_port` | Clone staged но не ушёл (short tx_burst в drain loop) **или** dropped at stage time из-за buffer full. Attention: mirror back-pressure mechanism именно здесь видно — peer DPI/IDS медленнее line rate. |
...
|---|---|---|---|
| `pktgate_watchdog_restarts_total` | C | — | **exposed: no** — watchdog/HA deferred. Count рестартов worker thread'а watchdog'ом. |
| `pktgate_bypass_active` | G | — | **exposed: no** — watchdog/HA deferred. `1` если pktgate в bypass mode (kernel-route в обход DPDK). |
...
| N2 — p99 latency | ≤500 µs overhead | **Phase 2**: `histogram_quantile(0.99, rate(pktgate_lcore_cycles_per_burst_bucket[5m])) / tsc_hz * 1e9`. В Phase 1 — bench-only (pre-ship artefacts), не live scrape. |
...
latency-бюджет валидируется через pre-ship bench-артефакты, а не через
