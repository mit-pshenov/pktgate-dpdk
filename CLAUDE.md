...
- **Exit-gate TSAN/ASAN evidence protocol** (post-M11 C1.5). Любой
  exit-gate commit, который упоминает dev-tsan или dev-asan test
  counts, ОБЯЗАН включать в тело коммита последние ~20 строк
  `ctest --preset dev-{tsan,asan} -L "..." -j 1 --output-on-failure`
  вывода. Naked «N/N GREEN» без evidence rejected at exit gate.
  При изменениях hot-path codegen (новый helper, atomics, memory
  ordering) добавляется objdump proof что `lock` prefix не появился.
  Прецедент: M10 C5 закрыли с claim «38/38 GREEN»; на деле TSAN
  race был, поймали через 3 дня на M11 C1 bisect. Canonical:
  `implementation-plan-errata.md §M11 C1.5 Protocol addendum`.
  Супервизорские handoff'ы должны surface'ить это как pre-commit
  checklist для worker'а.
- **Full 5-preset matrix на КАЖДОМ cycle close** (post-M15 C2
  amendment 2026-04-20). Каждый cycle close прогоняет полную
  матрицу `{dev-debug, dev-release, dev-asan, dev-ubsan, dev-tsan}`,
  не только exit-gate cycles. Исключение единственное — report-only
  cycles (C0 design notes с zero code diff) матрицу пропускают.
  Прецедент: M15 C1 закрыли на одном dev-debug preset (assumption
  «zero code change in resolver»), а TSan SEGV в
  `rte_eal_cleanup()` от vhost PMD pthread всплыл только на C2
  при полном прогоне. Proof of absence ≠ tests passed на одном
  пресете — live binary поменялся даже когда pktgate code не
  менялся. Canonical: `implementation-plan-errata.md §M11 C1.5
  Protocol amendment 2026-04-20`.
...
- **Правки в `/home/user/filter/`** — только по явной просьбе, это
  sibling-проект.
- **Git commit / push** — свободно: законченная работа (зелёные
  тесты) коммитится и пушится без напоминаний. Норма с июня 2026,
  в репозитории один владелец. TDD-циклы — авто-коммит на GREEN.
  Push в сторону dev VM может падать (ситуативный туннель) —
  авторитетный remote GitHub, на VM — rsync. Теги майлстоунов —
  по-прежнему по просьбе; частичный/сломанный цикл не коммитить
  (стиль сообщений — implementation-plan.md §0.3).
